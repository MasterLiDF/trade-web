...
export default function EditProductPage() {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const response = await httpFetch(`/api/product?id=${productId}`);
        if (response.success && response.data) {
          ...
          setCoverImageUrl(product.coverImage);
          setDetailImagePreviews(product.detailImages);
          setDetailImageUrls(product.detailImages);
        } else {
          alert('获取产品信息失败');
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await httpFetch('/api/product', {
        method: 'PUT',
        body: JSON.stringify({
          id: productId,
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500">修改皮革产品信息</p>
          </div>
        </div>
      </div>
